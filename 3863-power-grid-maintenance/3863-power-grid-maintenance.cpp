    ...
    vector<int> processQueries(int nodes, vector<vector<int>>& connections,
        ...
        for (int index = 0; index < size_q; ++index) {
            if (queries[index][0] == 2) {
                if (online[queries[index][1]]) online[queries[index][1]] = false;
                else queries[index][0] = 3;
            }
        }
        int groups[nodes + 1];
        ...
        for (int node = 1; node <= nodes; ++node) {
            ...
            groups[node] = group;
            if (online[node]) min_online[group] = min(min_online[group], node);
        }
        vector<int> result;
        for (int index = size_q - 1; index >= 0; --index) {
            if (queries[index][0] == 2) {
                ...
                if (online[queries[index][1]]) result.push_back(queries[index][1]);
                else if (min_online[groups[queries[index][1]]] == INT_MAX) result.push_back(-1);
                else result.push_back(min_online[groups[queries[index][1]]]);
            }
        }
