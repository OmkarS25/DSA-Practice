...
public:
    ...
    vector<vector<char>> rotateTheBox(vector<vector<char>> &box) {
        ...
        if (box.size() == 1ul && box.front().size() == 1ul) return box;

        // Go over rows.
        size_t row, nextStoneOrObstacle;
        int col;
            ...
            for (col = box.front().size()-1ul; col != -1; --col)
                switch (box[row][col]) {
                    case '#':
                        ...
                        break;
                    case '*': 
                        // Encountered obstacle - this is now the next obstacle or stone.
        ...
        vector<vector<char>> result(box.front().size(), vector<char>(box.size()));
        for (row = 0ul; row != box.size(); ++row)
