    ...
    vector<int> maxActiveSectionsAfterTrade(string s,
        ...
        for (auto q : queries) {
            ...
            if (gidx[l] == -1)
                ...
                left = glen[gidx[l]] - (l - gstart[gidx[l]]);

            int right;
            if (gidx[r] == -1)
                right = -1;
            else
                right = r - gstart[gidx[r]] + 1;
            ...
            if (fullStart > fullEnd) {
                ...
                if (s[l] == '0')
                    best = max(best, ones + left + glen[fullStart]);
                if (s[r] == '0')
                    best = max(best, ones + right + glen[fullEnd]);
                if (fullEnd > fullStart)
                    best = max(best, ones + query(0, 0, n - 1, fullStart, fullEnd - 1));
            }
            ans.push_back(best);
        }
        return ans;
